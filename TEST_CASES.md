...
## I. COA JSON Export Functionality Tests

1.  **Test Case EX-01: Export COA from Active Company**
    *   **Setup:** Open an existing company file that has a defined Chart of Accounts.
    *   **Action:**
        1.  Navigate to "File" -> "Export COA (JSON)".
        2.  Select a valid location and filename (e.g., `coa_export_test.json`).
        3.  Save the file.
    *   **Expected Result:**
        1.  An information alert confirms successful export.
        2.  The JSON file is created at the specified location.
        3.  The content of the JSON file accurately represents the company's COA structure and data (manual inspection of the JSON would be needed).

2.  **Test Case EX-02: Attempt Export with No Company Open**
    *   **Setup:** Ensure no company file is open.
    *   **Action:** Navigate to "File" menu.
    *   **Expected Result:** The "Export COA (JSON)" menu item should be disabled.
...
4.  **Test Case EX-04: Export Empty COA**
    ...
    *   **Expected Result:**
        ...
        2.  JSON file is created, representing an empty COA (e.g., `{"rootAccounts": []}`).

## II. COA JSON Import Functionality Tests

1.  **Test Case IM-01: Import COA into Active Company (Overwrite/Replace)**
    *   **Setup:**
        1.  Have a valid COA JSON file ready (e.g., `coa_export_test.json` from EX-01, or a new well-formed one).
        2.  Open an existing company file (it may or may not have an existing COA).
    *   **Action:**
        1.  Navigate to "File" -> "Import COA (JSON)".
        2.  Select the prepared COA JSON file.
    *   **Expected Result:**
...
2.  **Test Case IM-02: Attempt Import with No Company Open**
    *   **Setup:** Ensure no company file is open.
    *   **Action:** Navigate to "File" menu.
    *   **Expected Result:** The "Import COA (JSON)" menu item should be disabled.

3.  **Test Case IM-03: Import Malformed/Invalid JSON File**
    *   **Setup:**
        1.  Create a text file with invalid JSON content, save as `invalid_coa.json`.
        2.  Open an existing company file.
    *   **Action:**
        1.  Navigate to "File" -> "Import COA (JSON)".
        2.  Select `invalid_coa.json`.
    *   **Expected Result:**
...
4.  **Test Case IM-04: Import Valid JSON that is not a COA structure**
    *   **Setup:**
        1.  Create a valid JSON file that does not conform to the COA structure (e.g., `{"name": "test"}`). Save as `not_a_coa.json`.
        2.  Open an existing company file.
    *   **Action:**
        1.  Navigate to "File" -> "Import COA (JSON)".
        2.  Select `not_a_coa.json`.
    *   **Expected Result:**
...
1.  **Test Case SM-01: Menu Item States - No Company Open**
    ...
    *   **Expected Result:**
        ...
        *   "Import COA (JSON)": Disabled
        *   "Export COA (JSON)": Disabled
...
2.  **Test Case SM-02: Menu Item States - Company Open**
    ...
    *   **Expected Result:**
        ...
        *   "Import COA (JSON)": Enabled
        *   "Export COA (JSON)": Enabled
...
- JSON files created by the export function.
- Manually crafted JSON files for import testing (valid, invalid, empty).

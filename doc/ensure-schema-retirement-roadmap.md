# `Database.ensureSchema()` Retirement Roadmap

This document tracks the staged retirement of broad Java schema authority from:

```text
nonprofitbookkeeping.core.Database#ensureSchema()
```

The repository is a **Maven** Java project. Use Maven commands such as `mvn test` and `mvn -Dtest=... test`. Do **not** use Gradle commands.

## Overall Direction

The long-term database startup model is:
...
The desired end state is that fresh development databases are created by Flyway, validated by Hibernate/JPA, and do not depend on Java `CREATE TABLE`, `ALTER TABLE`, or schema-shaping logic in `Database.ensureSchema()`.

## Rules for Every Slice

Use this pattern for every schema group:

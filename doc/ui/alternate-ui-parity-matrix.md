...
| Settings route | `NavigationPane` + `PanelHost.create(SETTINGS)` (and classic menu Settings panel) | `MainWindowAlternate.openPanel(SETTINGS)` -> `PanelHost.show(SETTINGS)` -> `AlternateSettingsHost` wrapping `SettingsPanelFX` | implemented | Round 12 implements the embed/wrap strategy around classic `SettingsPanelFX` controls so alternate settings no longer render placeholder controls. Global alternate Save is intentionally unsupported until the classic private save flow is extracted; use the hosted Save Settings button, and keep shared settings-save command extraction as follow-up parity hardening. | P0 |

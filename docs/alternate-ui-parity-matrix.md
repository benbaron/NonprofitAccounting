...
| Alternate command center | Classic `MainWindow` menu/toolbar command discovery | `MainWindowAlternate` Command Center pane (icon rail + left-nav action) | partial | Adds grouped File/Run/Reports/Help command entry points and placeholders for report actions; still not full classic parity. | P0 |
...
| Reports menu shortcuts | `MainWindow.buildMenuBar()` Reports menu items | `Reports` nav button to `PanelHost.show(REPORTS_WORKSPACE)` | partial | Workspace route now uses shared reports workspace panel; report-specific menu actions are still not mapped in alternate command surfaces. | P1 |

-- Move reporting schedule configuration ownership from Database.ensureSchema() to Flyway.

-- Legacy non-Flyway databases can already have the pre-JPA account table.
-- Backfill the columns and indexes referenced by this migration before adding
-- reporting/schedule foreign keys to account(id).

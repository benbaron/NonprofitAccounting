...
public final class FileExportService
{
	...
	/** The Constant OFX_DATE. */
	private static final DateTimeFormatter OFX_DATE =
		DateTimeFormatter.ofPattern("yyyyMMdd");
	
	/** The Constant BANK_ACCOUNT_TYPES. */
	private static final EnumSet<AccountType> BANK_ACCOUNT_TYPES =
		EnumSet.of(AccountType.ASSET,
			AccountType.BANK,
			AccountType.CASH,
			AccountType.CHECKING,
			AccountType.MONEYMKRT);
	
	/** The Constant CREDIT_CARD_ACCOUNT_TYPES. */
	private static final EnumSet<AccountType> CREDIT_CARD_ACCOUNT_TYPES =
		EnumSet.of(AccountType.CREDIT,
	...
	public static final class ExportResult
	{
		...
		/**
		 * Instantiates a new export result.
		 *
		 * @param success the success
		 * @param transactionsWritten the transactions written
	...
	public static boolean isSupportedStatementAccount(Account account)
	{
		...
		return BANK_ACCOUNT_TYPES.contains(type) ||
			CREDIT_CARD_ACCOUNT_TYPES.contains(type);
		...
		StatementFormat format)
	{
		...
		{
			return new ExportResult(false, 0,
				"No company context available for export.");
		}
		
		if (account == null)
		{
			return new ExportResult(false, 0,
				"No account selected for export.");
		}
		
		if (format == null)
		{
			return new ExportResult(false, 0,
				"Export format must be specified.");
		}
		...
		if (!isSupportedStatementAccount(account))
		{
			return new ExportResult(false,
				0,
				"Account type '" +
					Optional.ofNullable(account.getAccountType())
		...
		{
			return new ExportResult(false, 0,
				"Company ledger is not available.");
		}
		...
		{
			return new ExportResult(false, 0,
				"No transactions available to export for this account.");
		}
		...
		BigDecimal runningBalance = Optional
			.ofNullable(account.getOpeningBalance()).orElse(BigDecimal.ZERO);
		
		for (AccountingTransaction tx : sorted)
		{
			AccountingEntry entry =
				findEntryForAccount(tx, account.getAccountNumber());
			...
			{
				LOGGER.debug(
					"Skipping transaction without valid date for OFX export: {}",
					tx.getMemo());
				continue;
			}
			...
			String name =
				firstNonBlank(tx.getToFrom(), tx.getMemo(), "Transaction");
		...
		{
			return new ExportResult(false, 0,
				"No dated transactions found for account '" +
					account.getName() + "'.");
		}
		
		BigDecimal ledgerBalance =
			Optional.ofNullable(account.totalAccountBalance(ledger))
		...
		ledgerBalance = ledgerBalance
			.setScale(Math.max(ledgerBalance.scale(), 2), RoundingMode.HALF_UP);
		
		String start =
			(startDate != null ? startDate : endDate).format(OFX_DATE);
		...
		try
		{
			...
			if (CREDIT_CARD_ACCOUNT_TYPES.contains(account.getAccountType()))
			{
				CreditCardInfo ccInfo =
					buildCreditCardInfo(account, profile, start, end);
			...
			else
			{
				BankAccountInfo bankInfo =
					buildBankInfo(account, profile, start, end);
				OfxV2Writer.writeOfxFile(destination.getAbsolutePath(),
					signon,
					statementEntries,
					bankInfo,
		...
		catch (Exception ex)
		{
			...
			return new ExportResult(false, 0,
				"Failed to write OFX/QFX file: " + ex.getMessage());
		}
	...
	{
		return Comparator
			.comparing((AccountingTransaction tx) -> parseDate(tx.getDate()),
				Comparator.nullsLast(Comparator.naturalOrder()))
			.thenComparing(tx -> Optional
				.ofNullable(tx.getBookingDateTimestamp()).orElse(0L));
	 ...
	 */
	private static AccountingEntry findEntryForAccount(AccountingTransaction tx,
	 ...
	 */
	private static BigDecimal determineSignedAmount(AccountingEntry entry,
		Account account)
	{
		BigDecimal amount =
			Optional.ofNullable(entry.getAmount()).orElse(BigDecimal.ZERO);
	 ...
	 * @return the string
	 */
	private static String determineTransactionType(String checkNumber,
		BigDecimal amount)
	{
	 ...
	 * @return the signon data
	 */
	private static SignonData buildSignon(Company company,
		StatementFormat format)
	{
		SignonData signon = new SignonData();
		CompanyProfileModel profile = company.getCompanyProfile();
		String companyName =
			(profile != null) ? clean(profile.getCompanyName()) : null;
		
		signon.language = "ENG";
		signon.org =
			Optional.ofNullable(companyName).orElse("NonprofitAccounting");
		signon.fid = (format == StatementFormat.QFX) ? "INTUIT" :
			Optional.ofNullable(companyName)
				.orElse("NONPROFIT");
		signon.message =
			(format == StatementFormat.QFX) ? "QFX Export" : "OFX Export";
		...
		String endDate)
	{
		...
		info.accountId =
			Optional.ofNullable(account.getAccountNumber()).orElse("0000");
	 ...
	 * @return the string
	 */
	private static String resolveCurrency(Account account,
		CompanyProfileModel profile)
	{
		...
		if (profile != null && profile.getBaseCurrency() != null &&
			profile.getBaseCurrency().length() == 3)
		{
	 ...
	 * @return the string
	 */
	private static String determineBankId(Account account,
		CompanyProfileModel profile)
	{

...
public class ReportConfigurationService {
    ...
    public ReportConfigurationService() {
        ...
        this.objectMapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS); // Or configure date format if needed
    }
    ...
    public void saveConfigurations(List<ReportConfiguration> configs, File companyDirectory) throws IOException {
        ...
        if (companyDirectory == null || !companyDirectory.isDirectory()) {
            LOGGER.severe("Invalid company directory provided for saving report configurations: " + companyDirectory);
        ...
        try {
            ...
            LOGGER.log(Level.SEVERE, "Error saving report configurations to " + configFile.getAbsolutePath(), e);
    ...
    public List<ReportConfiguration> loadConfigurations(File companyDirectory) {
        if (companyDirectory == null || !companyDirectory.isDirectory()) {
            LOGGER.warning("Invalid company directory provided for loading report configurations: " + companyDirectory);
            return new ArrayList<>();
        }

        File configFile = new File(companyDirectory, CONFIG_FILE_NAME);
        if (!configFile.exists() || !configFile.isFile() || configFile.length() == 0) {
            LOGGER.info("Report configurations file not found or is empty. Returning empty list. Path: " + configFile.getAbsolutePath());
            return new ArrayList<>();
        }

        try {
            List<ReportConfiguration> configs = objectMapper.readValue(configFile, new TypeReference<List<ReportConfiguration>>() {});
            LOGGER.info("Report configurations loaded successfully from: " + configFile.getAbsolutePath());
            // Ensure IDs are present (though constructor should handle for new ones)
            configs.forEach(config -> {
                if (config.getConfigurationId() == null || config.getConfigurationId().trim().isEmpty()) {
                    // This case should ideally not happen if configs are always saved with IDs.
                    // For robustness, could assign a new one, but log a warning as it implies data issue.
                    LOGGER.warning("Loaded a ReportConfiguration with a missing or empty ID. UserGivenName: " + config.getUserGivenName());
                    // Example: if(config.getConfigurationId() == null) config.setConfigurationId(UUID.randomUUID().toString());
                    // However, modifying loaded data might be unexpected. Logging is safer.
                }
            });
            return configs;
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE, "Error loading report configurations from " + configFile.getAbsolutePath(), e);

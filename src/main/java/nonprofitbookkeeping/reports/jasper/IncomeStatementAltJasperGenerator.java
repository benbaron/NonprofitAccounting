...
public class IncomeStatementAltJasperGenerator extends AbstractReportGenerator
{
	 ...
	 */
	@Override
	protected List<IncomeStatementAltRowBean> getReportData()
	{
		...
		java.util.Map<String, java.math.BigDecimal> totals =
			new java.util.HashMap<>();
		
		for (nonprofitbookkeeping.model.AccountingTransaction tx : company
			.getLedger()
			.getTransactions())
		{
				...
				.getEntries())
			{
				...
				if (acct.getAccountType() ==
					nonprofitbookkeeping.model.AccountType.INCOME ||
					acct.getAccountType() ==
						nonprofitbookkeeping.model.AccountType.EXPENSE)
				{
					java.math.BigDecimal amt =
						totals.getOrDefault(acct.getName(),
							java.math.BigDecimal.ZERO);
					
					if ((acct.getAccountType() ==
						nonprofitbookkeeping.model.AccountType.INCOME &&
						entry.getAccountSide() ==
							nonprofitbookkeeping.model.AccountSide.CREDIT) ||
						(acct.getAccountType() ==
							nonprofitbookkeeping.model.AccountType.EXPENSE &&
							entry.getAccountSide() ==
		...
		java.util.List<IncomeStatementAltRowBean> rows =
			new java.util.ArrayList<>();
		
		for (java.util.Map.Entry<String, java.math.BigDecimal> e : totals
			.entrySet())
		{

    ...
    protected List<DEPR_DTL_8cBean> getReportData()
    {
        ...
        try
        {
            selectList = FieldMapSqlBuilder.buildSelectList(
                "/nonprofitbookkeeping/reports/DEPR_DTL_8c_fieldmap.csv",
                overrides
            );
        }
        catch (IOException ex)
        {
            throw new IllegalStateException(
                "Unable to load DEPR_DTL_8c field map", ex);
        }
        ...
        return ReportDataFetcher.queryBeans(DEPR_DTL_8cBean.class, sql);
    }

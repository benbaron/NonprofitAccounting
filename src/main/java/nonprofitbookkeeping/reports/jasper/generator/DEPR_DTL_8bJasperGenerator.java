    ...
    protected List<DEPR_DTL_8bBean> getReportData()
    {
        ...
        try
        {
            selectList = FieldMapSqlBuilder.buildSelectList(
                "/nonprofitbookkeeping/reports/DEPR_DTL_8b_fieldmap.csv",
                overrides
            );
        }
        catch (IOException ex)
        {
            throw new IllegalStateException(
                "Unable to load DEPR_DTL_8b field map", ex);
        }
        ...
        return ReportDataFetcher.queryBeans(DEPR_DTL_8bBean.class, sql);
    }

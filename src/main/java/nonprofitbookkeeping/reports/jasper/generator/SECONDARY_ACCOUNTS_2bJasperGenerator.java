    ...
    protected List<SECONDARY_ACCOUNTS_2bBean> getReportData()
    {
        ...
        try
        {
            selectList = FieldMapSqlBuilder.buildSelectList(
                "/nonprofitbookkeeping/reports/SECONDARY_ACCOUNTS_2b_fieldmap.csv",
                overrides
            );
        }
        catch (IOException ex)
        {
            throw new IllegalStateException(
                "Unable to load SECONDARY_ACCOUNTS_2b field map", ex);
        }
        ...
        return ReportDataFetcher.queryBeans(SECONDARY_ACCOUNTS_2bBean.class, sql);
    }

    ...
    protected List<TRANSFER_IN_9Bean> getReportData()
    {
        ...
        try
        {
            selectList = FieldMapSqlBuilder.buildSelectList(
                "/nonprofitbookkeeping/reports/TRANSFER_IN_9_fieldmap.csv",
                overrides
            );
        }
        catch (IOException ex)
        {
            throw new IllegalStateException(
                "Unable to load TRANSFER_IN_9 field map", ex);
        }
        ...
        return ReportDataFetcher.queryBeans(TRANSFER_IN_9Bean.class, sql);
    }

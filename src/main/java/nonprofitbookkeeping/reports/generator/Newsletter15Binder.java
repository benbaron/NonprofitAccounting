...
public class Newsletter15Binder
{
	...
	private static Newsletter15Bean buildBean()
	{
		...
		bean.getRows().add(new Newsletter15Row("Acme Widgets", "Full",
			"Jan-Feb", new BigDecimal("50.00"), "123", "2025-01-01"));
		return bean;
		...
		Newsletter15Bean bean) throws Exception
	{
		...
		params.put("P_TOTAL_ADS",
			sum(bean.getRows(), Newsletter15Row::getAmount));
		
		try (InputStream in =
			Newsletter15Binder.class.getResourceAsStream(jrxmlOnClasspath))
		{

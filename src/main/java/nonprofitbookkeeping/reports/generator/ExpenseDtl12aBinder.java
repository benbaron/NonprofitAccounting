...
public class ExpenseDtl12aBinder
{
	...
	private static List<AdvertisingExpenseRow> buildAdvertisingRows()
	{
		return Arrays.asList(
			new AdvertisingExpenseRow("A1", "SCA Times",
				new BigDecimal("125.00")),
			new AdvertisingExpenseRow("A2", "Local Herald",
	...
	{
		return Arrays.asList(
			new BadDebtRow("B1", "John Smith", "Uncollectible site fee",
	...
	{
		return Arrays.asList(
			new FeeHonorariumRow("F1", "Jane Doe", "Musician for court",
		...
		Collection<FeeHonorariumRow> feeRows) throws Exception
	{
		...
		try (InputStream in =
			ExpenseDtl12aBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{

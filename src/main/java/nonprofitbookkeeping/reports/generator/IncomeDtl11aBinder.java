...
import nonprofitbookkeeping.reports.datasource.scareports.DemoActivityIncomeRow;
import nonprofitbookkeeping.reports.datasource.scareports.DirectContributionRow;
import nonprofitbookkeeping.reports.datasource.scareports.FundraisingIncomeExternalRow;
import nonprofitbookkeeping.reports.datasource.scareports.FundraisingIncomeInternalRow;
import nonprofitbookkeeping.reports.datasource.scareports.Income11aRow;
...
public class IncomeDtl11aBinder
{
	...
	private static List<FundraisingIncomeInternalRow> buildInternalRows()
	{
		return Arrays.asList(
			new FundraisingIncomeInternalRow("Crown Tournament", "Bake sale",
	...
	{
		return Arrays.asList(
			new FundraisingIncomeExternalRow("Local Fair", "Demo booth",
	...
	{
		return Arrays.asList(
			new DirectContributionRow("Jane Doe", "General fund",
	...
	{
		return Arrays.asList(
			new DemoActivityIncomeRow("City Parade", "Fighter demo",
		...
		Collection<DemoActivityIncomeRow> demoRows) throws Exception
	{
		...
		BigDecimal internalTotal =
			sum(internalRows, FundraisingIncomeInternalRow::getAmount);
		BigDecimal externalTotal =
			sum(externalRows, FundraisingIncomeExternalRow::getAmount);
		BigDecimal contributionTotal =
			sum(contributionRows, DirectContributionRow::getAmount);
		...
		try (InputStream in =
			IncomeDtl11aBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{

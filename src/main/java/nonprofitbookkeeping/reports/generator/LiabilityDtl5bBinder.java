...
import nonprofitbookkeeping.reports.datasource.scareports.DeferredRevenueRow;
import nonprofitbookkeeping.reports.datasource.scareports.LiabilityDtl5bRow;
import nonprofitbookkeeping.reports.datasource.scareports.OtherLiabilityRow;
import nonprofitbookkeeping.reports.datasource.scareports.PayableRow;
...
public class LiabilityDtl5bBinder
{
	...
	private static List<DeferredRevenueRow> buildDeferredRevenueRows()
	{
		...
		list.add(new DeferredRevenueRow("Regional Event",
			new BigDecimal("100.00"), new BigDecimal("150.00")));
		list.add(new DeferredRevenueRow("Winter Gala", new BigDecimal("200.00"),
	...
	private static List<PayableRow> buildPayableRows()
	{
		...
		list.add(new PayableRow("Acme Supplies", "Site materials",
			new BigDecimal("75.00"), new BigDecimal("0.00")));
		list.add(new PayableRow("John Doe", "Travel reimbursement",
			new BigDecimal("0.00"), new BigDecimal("120.00")));
		return list;
	...
	private static List<OtherLiabilityRow> buildOtherLiabilityRows()
	{
		...
		list.add(new OtherLiabilityRow("Event Insurance", "Deposit",
		...
		Collection<OtherLiabilityRow> otherRows) throws Exception
	{
		...
		BigDecimal deferredPriorTotal =
			sum(deferredRows, DeferredRevenueRow::getPriorAmount);
		BigDecimal deferredCurrentTotal =
			sum(deferredRows, DeferredRevenueRow::getCurrentAmount);
		BigDecimal payablePriorTotal =
			sum(payableRows, PayableRow::getPriorAmount);
		BigDecimal payableCurrentTotal =
			sum(payableRows, PayableRow::getCurrentAmount);
		BigDecimal otherPriorTotal =
			sum(otherRows, OtherLiabilityRow::getPriorAmount);
		BigDecimal otherCurrentTotal =
			sum(otherRows, OtherLiabilityRow::getCurrentAmount);
		...
		try (InputStream in =
			LiabilityDtl5bBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{
	...
	public static void main(String[] args) throws Exception
	{
		...
		JasperPrint print = fillLiabilityDtl5b(jrxml, "Sample Org",
			"Liability Detail Worksheet", deferred, payables, others);
		JasperExportManager.exportReportToPdfFile(print,
			"LIABILITY_DTL_5b.pdf");

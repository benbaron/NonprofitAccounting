...
import nonprofitbookkeeping.reports.datasource.scareports.PrimaryAccountCheckRow;
import nonprofitbookkeeping.reports.datasource.scareports.PrimaryAccountDepositRow;
...
public class PrimaryAccount2aBinder
{
	...
	private static List<PrimaryAccountDepositRow> buildDepositRows()
	{
		...
		list.add(new PrimaryAccountDepositRow("2025-01-05",
			new BigDecimal("100.00")));
		list.add(new PrimaryAccountDepositRow("2025-01-15",
	...
	private static List<PrimaryAccountCheckRow> buildCheckRows()
	{
		...
		list.add(new PrimaryAccountCheckRow("101", "2025-01-07",
			new BigDecimal("75.00")));
		list.add(new PrimaryAccountCheckRow("102", "2025-01-20",
		...
		Collection<PrimaryAccountCheckRow> checkRows) throws Exception
	{
		...
		// Totals
		params.put("P_DEPOSIT_TOTAL",
			sum(depositRows, PrimaryAccountDepositRow::getAmount));
		params.put("P_CHECK_TOTAL",
			sum(checkRows, PrimaryAccountCheckRow::getAmount));
		
		try (InputStream in =
			PrimaryAccount2aBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{
	...
	{
		String jrxml =
			"/jrxml/sca-reports/PRIMARY_ACCOUNT_2a_fixed_labeled.jrxml";
		List<PrimaryAccountDepositRow> deposits = buildDepositRows();
		List<PrimaryAccountCheckRow> checks = buildCheckRows();
		
		JasperPrint print = fillPrimaryAccount2a(jrxml, "First Bank",
			"Checking", deposits, checks);
		JasperExportManager.exportReportToPdfFile(print,
			"PRIMARY_ACCOUNT_2a.pdf");

...
import nonprofitbookkeeping.reports.datasource.scareports.AssetDtl5aRow;
import nonprofitbookkeeping.reports.datasource.scareports.OtherAssetRow;
import nonprofitbookkeeping.reports.datasource.scareports.PrepaidExpenseRow;
import nonprofitbookkeeping.reports.datasource.scareports.ReceivableRow;
import nonprofitbookkeeping.reports.datasource.scareports.UndepositedFundsRow;
...
public class AssetDtl5aBinder
{
	...
	private static List<UndepositedFundsRow> buildUndepositedFundRows()
	{
		...
		list.add(new UndepositedFundsRow("Event gate cash",
			new BigDecimal("50.00")));
		list.add(new UndepositedFundsRow("Fundraiser proceeds",
	...
	private static List<ReceivableRow> buildReceivableRows()
	{
		...
		list.add(new ReceivableRow("John Doe", "Site advance",
			new BigDecimal("100.00"), new BigDecimal("0.00")));
		list.add(new ReceivableRow("Jane Smith", "Reimbursement",
			new BigDecimal("0.00"), new BigDecimal("75.00")));
		return list;
	...
	private static List<PrepaidExpenseRow> buildPrepaidExpenseRows()
	{
		...
		list.add(new PrepaidExpenseRow("Site deposit", new BigDecimal("200.00"),
			new BigDecimal("0.00")));
		list.add(new PrepaidExpenseRow("Storage rent", new BigDecimal("50.00"),
			new BigDecimal("50.00")));
		return list;
	...
	private static List<OtherAssetRow> buildOtherAssetRows()
	{
		...
		list.add(new OtherAssetRow("Security deposit", new BigDecimal("0.00"),
	...
	{
		// Totals for each section
		BigDecimal undepositedTotal =
			sum(undepositedRows, UndepositedFundsRow::getAmount);
		BigDecimal receivablePriorTotal =
			sum(receivableRows, ReceivableRow::getPriorAmount);
		BigDecimal receivableCurrentTotal =
			sum(receivableRows, ReceivableRow::getCurrentAmount);
		BigDecimal prepaidPriorTotal =
			sum(prepaidRows, PrepaidExpenseRow::getPriorAmount);
		BigDecimal prepaidCurrentTotal =
			sum(prepaidRows, PrepaidExpenseRow::getCurrentAmount);
		BigDecimal otherPriorTotal =
			sum(otherAssetRows, OtherAssetRow::getPriorAmount);
		BigDecimal otherCurrentTotal =
			sum(otherAssetRows, OtherAssetRow::getCurrentAmount);
		...
		try (InputStream in =
			AssetDtl5aBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{
	...
	public static void main(String[] args) throws Exception
	{
		...
		JasperPrint print = fillAssetDtl5a(jrxml, "Sample Org",
			"Asset Detail Worksheet", undep, recs, prepaids, others);

...
import nonprofitbookkeeping.reports.datasource.scareports.AdjustedGrossEventIncomeRow;
import nonprofitbookkeeping.reports.datasource.scareports.Income11bRow;
import nonprofitbookkeeping.reports.datasource.scareports.NetAdvertisingIncomeRow;
import nonprofitbookkeeping.reports.datasource.scareports.OtherIncomeRow;
...
public class IncomeDtl11bBinder
{
	...
	private static List<AdjustedGrossEventIncomeRow> buildEventRows()
	{
		return Arrays.asList(
			new AdjustedGrossEventIncomeRow("Coronation", "150 attendees",
	...
	{
		return Arrays.asList(
			new NetAdvertisingIncomeRow("Newsletter ads", "3 issues",
		...
		Collection<OtherIncomeRow> otherRows) throws Exception
	{
		...
		BigDecimal eventTotal =
			sum(eventRows, AdjustedGrossEventIncomeRow::getAmount);
		BigDecimal advertisingTotal =
			sum(advertisingRows, NetAdvertisingIncomeRow::getAmount);
		...
		try (InputStream in =
			IncomeDtl11bBinder.class.getResourceAsStream(jrxmlOnClasspath))
		{

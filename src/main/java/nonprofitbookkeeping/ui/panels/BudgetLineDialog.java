...
public class BudgetLineDialog extends JDialog {
    ...
    private static class FundItem {
        ...
        public FundItem(Fund fund) { this.fund = fund; this.displayName = fund.getName();}
        public FundItem(String displayName) { this.displayName = displayName; this.fund = null;} // For "None"
        public Fund getFund() { return fund; }
        public String getFundId() { return fund != null ? fund.getFundId() : null; }
        @Override public String toString() { return displayName; }
         @Override public boolean equals(Object o) {
            ...
            return Objects.equals(displayName, that.displayName); // Compare by name if one or both funds are null (e.g. "None")
        }
        @Override public int hashCode() {
             if (fund != null) return Objects.hash(fund.getFundId());
             return Objects.hash(displayName);
        }
    }


    public BudgetLineDialog(Dialog owner, String title, ChartOfAccounts coa, List<Fund> funds, BudgetLine existingLine) {
        ...
        if (this.budgetLine.getPeriodicity() == null) { // Ensure default if new line
    ...
    private void attachListeners() {
        // Listeners can be added if dynamic interactions are needed before OK is pressed
    }
    ...
    private void saveAndClose() {
        ...
        if (amountStr.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Total Budgeted Amount cannot be empty.", "Input Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (totalAmount.compareTo(BigDecimal.ZERO) < 0) {
                 JOptionPane.showMessageDialog(this, "Total Budgeted Amount cannot be negative.", "Input Error", JOptionPane.ERROR_MESSAGE);
                 return;
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Invalid number format for Total Budgeted Amount.", "Input Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        budgetLine.setAccountId(selectedAccount.getAccountNumber()); // Assuming getAccountNumber() is the ID

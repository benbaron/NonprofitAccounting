...
public class BudgetLineTableModel extends AbstractTableModel {
    ...
    private final String[] columnNames = {"Account Name", "Total Budgeted Amount", "Periodicity", "Line Fund"};

    public BudgetLineTableModel(List<BudgetLine> budgetLines, ChartOfAccounts chartOfAccounts, List<Fund> availableFunds) {
    ...
    public Class<?> getColumnClass(int columnIndex) {
        switch (columnIndex) {
            case 0: return String.class; // Account Name
            case 1: return BigDecimal.class; // Total Budgeted Amount
            case 2: return Periodicity.class; // Periodicity
            case 3: return String.class; // Line Fund (Name)
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        BudgetLine line = budgetLines.get(rowIndex);
        switch (columnIndex) {
            case 0: // Account Name
                if (line.getAccountId() != null && chartOfAccounts != null) {
                    Account account = chartOfAccounts.getAccount(line.getAccountId());
            ...
            case 3: // Line Fund
                if (line.getFundId() != null && !availableFunds.isEmpty()) {
                    return availableFunds.stream()
                            .filter(fund -> line.getFundId().equals(fund.getFundId())) // Assuming Fund has getFundId()
            ...
            case 1: // Total Budgeted Amount
                if (aValue instanceof BigDecimal) {
                    ...
                    try {
                        ...
                        // Handle error or ignore - JTable default editor for BigDecimal might prevent this
    ...
    public void addRow(BudgetLine line) {
        ...
        fireTableRowsInserted(budgetLines.size() - 1, budgetLines.size() - 1);
    }

    public void removeRow(int rowIndex) {
        if (rowIndex >= 0 && rowIndex < budgetLines.size()) {
            budgetLines.remove(rowIndex);
    ...
    public BudgetLine getBudgetLineAt(int rowIndex) {
        if (rowIndex >= 0 && rowIndex < budgetLines.size()) {
            return budgetLines.get(rowIndex);
        }
        return null;
    }

...
public class AccountTransactionDetailsPanelFX extends BorderPane {
    ...
    public AccountTransactionDetailsPanelFX() {
        ...
        if (company != null && company.getChartOfAccounts() != null) {
            ...
            if (coa.getAccounts() != null) {
                ...
                Callback<ListView<Account>, ListCell<Account>> cellFactory = lv -> new ListCell<Account>() {
                    @Override
                    protected void updateItem(Account item, boolean empty) {
                        super.updateItem(item, empty);
                        setText(empty ? null : item.getName() + " (" + item.getAccountNumber() + ")");
                    }
                };
        ...
        accountSelectorComboBox.setOnAction(e -> {
            ...
            transactionsTable.setPlaceholder(new Label("Account selection changed. Click 'Load Transactions'."));
        ...
        transactionsTable.setPlaceholder(new Label("Select account and date range, then click 'Load Transactions'."));
    ...
    private void setupTableColumns() {
        ...
        TableColumn<TransactionDisplayRow, BigDecimal> balanceCol = new TableColumn<>("Running Balance");
        ...
        transactionsTable.getColumns().addAll(dateCol, idCol, descCol, debitCol, creditCol, balanceCol);
    }
    ...
    private void loadTransactionData() {
        ...
        if (startDate == null || endDate == null) {
            AlertBox.showError(this.getScene().getWindow(), "Please select both a start and end date.");
            return;
        }
        if (endDate.isBefore(startDate)) {
            AlertBox.showError(this.getScene().getWindow(), "End date cannot be before start date.");
            return;
        }
        ...
        BigDecimal runningBalance = selectedAccount.getOpeningBalance() != null ? selectedAccount.getOpeningBalance() : BigDecimal.ZERO;
                ...
                try {
                    ...
                    if (txDate.isBefore(startDate)) {
                        for (AccountingEntry entry : tx.getEntries()) {
                            if (entry.getAccount() != null && entry.getAccount().getAccountNumber().equals(selectedAccount.getAccountNumber())) {
                    ...
                    System.err.println("Could not parse transaction date for opening balance calc: " + tx.getDate() + " for TX ID: " + tx.getBookingDateTimestamp());
                }
            }
        }
        ...
        if (ledger.getTransactions() != null) {
            ...
            for (AccountingTransaction tx : ledger.getTransactions()) { // Iterate again, or use the sorted list if performance allows
                if (tx.getDate() == null || tx.getDate().trim().isEmpty()) continue;
                try {
                     LocalDate txDate = LocalDate.parse(tx.getDate());
            ...
            for (AccountingTransaction tx : periodTransactions) {
                for (AccountingEntry entry : tx.getEntries()) {
                    if (entry.getAccount() != null && entry.getAccount().getAccountNumber().equals(selectedAccount.getAccountNumber())) {
                        ...
                        BigDecimal entryAmount = entry.getAmount() != null ? entry.getAmount() : BigDecimal.ZERO;
        ...
        if(transactionDataList.isEmpty()){
            transactionsTable.setPlaceholder(new Label("No transactions found for the selected account and date range."));
        } else {
    ...
    private void setupCompanyChangeListener() {
       companyChangeListener = new CompanyChangeListener() {
           ...
           public void companyChange(boolean companyNowOpen) {
               transactionDataList.clear();
               transactionsTable.setPlaceholder(new Label("Company changed. Select account and date range, then click 'Load Transactions'."));
               totalDebitsLabel.setText("Total Debits: 0.00");
               totalCreditsLabel.setText("Total Credits: 0.00");
               netChangeLabel.setText("Net Change: 0.00");

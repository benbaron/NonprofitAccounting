...
public class SkeletonJournalPanel extends BorderPane
{
	...
	public SkeletonJournalPanel()
	{
		...
		filterControlsBox.getChildren().addAll(filterLabel, searchFilterField, dateFilterField,
			applyFilterButton);
	...
	private void setupTableColumns()
	{
		...
		journalDisplayTable.getColumns().addAll(dateCol, transIdCol, accountCol, descCol, debitCol,
			creditCol);
	}
	...
	private void setupEventListenersAndRefresh()
	{
		...
		applyFilterButton.setOnAction(e -> {
			...
			System.out.println("Filter button clicked. Search: " + searchFilterField.getText() +
				", Date: " + dateFilterField.getText());
			loadData();
		});
		...
		editEntryButton.setOnAction(e -> {
			JournalDisplayEntry selected =
				journalDisplayTable.getSelectionModel().getSelectedItem();

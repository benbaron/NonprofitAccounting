...
public class BudgetPanel extends JDialog {
    ...
    public BudgetPanel(Frame owner, ChartOfAccounts chartOfAccounts, List<Fund> funds, 
        ...
        if (budgetToEdit != null) {
            ...
            if (CurrentCompany.getCompany() != null && CurrentCompany.getCompany().getCompanyProfile() != null) {
                this.currentBudget.setCurrency(CurrentCompany.getCompany().getCompanyProfile().getCurrency());
            } else {
    ...
    private void populateUIFromCurrentBudget() {
        ...
        // If not, we would need: budgetLineTableModel = new BudgetLineTableModel(...); tblBudgetLines.setModel(...);
    }

    private void initComponents() {
        // Budget Properties - Initialize them first, then populate in populateUIFromCurrentBudget
        txtBudgetName = new JTextField(20);
        spnFiscalYear = new JSpinner(new SpinnerNumberModel(LocalDate.now().getYear(), 2000, 2100, 1));
        ...
        // Budget Lines Table
        // Ensure currentBudget.getBudgetLines() is not null if currentBudget could be partially constructed
        if (currentBudget.getBudgetLines() == null) {
            currentBudget.setBudgetLines(new ArrayList<>());
        }
        budgetLineTableModel = new BudgetLineTableModel(currentBudget.getBudgetLines(), chartOfAccounts, availableFunds);
        ...
        TableColumn periodicityColumn = tblBudgetLines.getColumnModel().getColumn(2); // Assuming index 2 for Periodicity
        ...
        // Using fundNames vector which includes "All Funds" or a "None" option if appropriate
        Vector<String> lineFundNames = new Vector<>(fundNames); // Copy, can be modified if needed
        if (!lineFundNames.contains("None")) lineFundNames.add(0, "None"); // Option for no specific fund on a line
        JComboBox<String> lineFundComboBox = new JComboBox<>(lineFundNames);
        TableColumn lineFundColumn = tblBudgetLines.getColumnModel().getColumn(3); // Assuming index 3 for Line Fund
    ...
    private void layoutComponents() {
        ...
        gbc.gridx = 0; gbc.gridy = 0; pnlProperties.add(new JLabel("Budget Name:"), gbc);
        gbc.gridx = 1; pnlProperties.add(txtBudgetName, gbc);
        gbc.gridx = 0; gbc.gridy = 1; pnlProperties.add(new JLabel("Fiscal Year:"), gbc);
        gbc.gridx = 1; pnlProperties.add(spnFiscalYear, gbc);
        gbc.gridx = 0; gbc.gridy = 2; pnlProperties.add(new JLabel("Description:"), gbc);
        gbc.gridx = 1; pnlProperties.add(txtDescription, gbc);
        gbc.gridx = 0; gbc.gridy = 3; pnlProperties.add(new JLabel("Applicable Fund:"), gbc);
        gbc.gridx = 1; pnlProperties.add(cmbApplicableFund, gbc);
        gbc.gridx = 0; gbc.gridy = 4; pnlProperties.add(new JLabel("Currency:"), gbc);
        gbc.gridx = 1; pnlProperties.add(txtCurrency, gbc);
    ...
    private void attachListeners() {
        ...
        spnFiscalYear.addChangeListener(e -> currentBudget.setFiscalYear((Integer) spnFiscalYear.getValue()));
        txtDescription.addActionListener(e -> currentBudget.setDescription(txtDescription.getText()));
    ...
    private void actionAddLine(ActionEvent e) {
        BudgetLineDialog dialog = new BudgetLineDialog(this, "Add Budget Line", chartOfAccounts, availableFunds, null);
    ...
    private void actionEditLine(ActionEvent e) {
        ...
        if (selectedRow >= 0) {
            ...
            BudgetLineDialog dialog = new BudgetLineDialog(this, "Edit Budget Line", chartOfAccounts, availableFunds, lineToEdit);
            dialog.setVisible(true);
            if (dialog.isSaved()) {
            ...
            JOptionPane.showMessageDialog(this, "Please select a line to edit.", "No Selection", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void actionRemoveLine(ActionEvent e) {
        int selectedRow = tblBudgetLines.getSelectedRow();
        if (selectedRow >= 0) {
            if (JOptionPane.showConfirmDialog(this, "Are you sure you want to remove this line?", "Confirm Remove", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                budgetLineTableModel.removeRow(selectedRow);
                // currentBudget.getBudgetLines() is directly modified by table model
            }
        } else {
            JOptionPane.showMessageDialog(this, "Please select a line to remove.", "No Selection", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void actionSaveBudget(ActionEvent e) {
        ...
        // currentBudget.setBudgetLines() is already managed by the table model directly modifying the list.
        ...
        try {
            ...
            ex.printStackTrace(); 
            JOptionPane.showMessageDialog(this, "Error saving budget: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    // Main method for testing (optional, needs adaptation for new constructor)
    public static void main(String[] args) {
        // This main method needs to be updated to provide BudgetService and File companyDirectory.
        // For basic UI testing, it can be simplified or dependencies can be mocked/faked.
        // Example:
        ...
        //      nonprofitbookkeeping.model.Company testCompany = new nonprofitbookkeeping.model.Company();
        //      nonprofitbookkeeping.model.CompanyProfileModel profile = new nonprofitbookkeeping.model.CompanyProfileModel();
        ...
        //     BudgetPanel panel = new BudgetPanel(null, coa, funds, mockBudgetService, mockCompanyDir, budgetToEdit);
        //     panel.setVisible(true);
        // });
        System.out.println("BudgetPanel main method needs adaptation for new constructor dependencies (BudgetService, companyDirectory).");
    }
}

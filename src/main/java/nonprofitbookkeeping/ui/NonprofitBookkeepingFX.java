...
public class NonprofitBookkeepingFX extends Application {
	...
	private static final class ServiceContainer {
		...
		private static final ReportConfigurationService reportConfigurationService = new ReportConfigurationService();
        ...
        // FileImportService and FileExportService are typically used via Actions, not directly managed as singletons here.
	}
	...
	@Override public void start(Stage stage) {
        ...
        for (Plugin plugin : pluginLoader) {
            try {
                ...
                LOGGER.log(Level.SEVERE, "Failed to initialize plugin: " + plugin.getClass().getName() + " - " + e.getMessage(), e);
                AlertBox.showError(this.primaryStage, "Plugin Load Error", "Failed to initialize plugin: " + plugin.getName() + "\n" + e.getMessage());
            }
        }
        LOGGER.info("Plugin discovery complete. Loaded " + this.loadedPlugins.size() + " plugins.");

        // MenuBar must be built *after* plugins are loaded so they can add their items.
	...
	private MenuBar buildMenuBar() {
		...
		add(edit, "Open Budget Editor", e -> {
            ...
            if (currentCompany == null) {
                AlertBox.showError(this.primaryStage, "No company open."); return;
            }
            File companyDir = (currentCompany.getCompanyFile() != null) ? currentCompany.getCompanyFile().getParentFile() : null;
            if (companyDir == null) {
                 AlertBox.showError(this.primaryStage, "Company directory not found. Cannot manage budgets."); return;
            }
		...
		add(this.run, "Documents & Attachments", e -> showPanel(new DocumentsPanelFX(ServiceContainer.dss), "Documents"));
		add(this.run, "Inventory & Depreciation", e -> showPanel(new InventoryPanelFX(ServiceContainer.iss), "Inventory"));
		add(this.run, "Funds & Fund Accounting", e -> showPanel(new FundsPanelFX(ServiceContainer.fas), "Funds"));
		add(this.run, "Reconcile", e -> showPanel(new ReconcilePanelFX(new ReconciliationService()), "Reconciliation"));
		// Note: The SCA Ledger submenu was here. It will be added by the SCALedgerPlugin if loaded.
		bar.getMenus().add(this.run);
		
		/* REPORTS */
		this.reports = new Menu("Reports");
		// add(this.reports, "Generate Reports", e -> { /* implement */}); // Old placeholder
		add(this.reports, "Show Reports", e -> showPanel(new ReportsPanelFX(), "Reports"));
		add(this.reports, "Show Accounts", e -> showPanel(new AccountsPanelFX(new AccountService()), "Chart of Accounts"));
		add(this.reports, "Show Account Activity", e -> {
            Company currentCompany = CurrentCompany.getCompany();
            if (currentCompany != null && currentCompany.getLedger() != null) {
                showPanel(new AccountsActivityPanelFX(currentCompany.getLedger()), "Account Activity");
            } else {
                AlertBox.showError(this.primaryStage, "No company or ledger open.");
            }
        });
		add(this.reports, "Generate Income Statement", e -> new GenerateIncomeStatementAction(ServiceContainer.reportService).actionPerformed(null));
		add(this.reports, "Generate Balance Sheet", e -> new GenerateBalanceSheetAction(ServiceContainer.reportService).actionPerformed(null));
		add(this.reports, "Generate Trial Balance", e -> new GenerateTrialBalanceAction(ServiceContainer.reportService).actionPerformed(null));
		add(this.reports, "Generate Cash Flow Statement", e -> new GenerateCashFlowStatementAction(ServiceContainer.reportService).actionPerformed(null));
		add(this.reports, "Generate Budget vs. Actuals Report", e -> new GenerateBudgetVsActualsReportAction(ServiceContainer.reportService, ServiceContainer.budgetService).actionPerformed(null));
        add(this.reports, "Manage Saved Reports", e -> {
            Company currentCompany = CurrentCompany.getCompany();
            if (currentCompany == null || currentCompany.getCompanyFile() == null || currentCompany.getCompanyFile().getParentFile() == null) {
                AlertBox.showError(this.primaryStage, "Company context not properly set for managing reports."); return;
            }
        ...
        add(this.reports, "Generate Account Activity Detail", e -> new GenerateAccountActivityReportAction(ServiceContainer.reportService).actionPerformed(null));
		...
		add(this.panels, "Donations", e -> showPanel(new DonationsPanelFX(this.primaryStage), "Donations"));
		add(this.panels, "Grants", e -> showPanel(new GrantsPanelFX(this.primaryStage), "Grants"));
		add(this.panels, "Sales & COG", e -> showPanel(new SalesAndCOGPanelFX(this.primaryStage), "Sales & COG"));
		bar.getMenus().add(this.panels);
		
		/* SETTINGS */
		Menu settings = new Menu("Settings");
		add(settings, "Show Settings", e -> showPanel(new SettingsPanelFX(this.primaryStage), "Settings"));
        ...
        LOGGER.info("Adding plugin menu items. Number of plugins: " + (this.loadedPlugins != null ? this.loadedPlugins.size() : 0));
        if (this.loadedPlugins != null) {
            for (Plugin plugin : this.loadedPlugins) {
                try {
                    ...
                    LOGGER.log(Level.WARNING, "Plugin " + plugin.getName() + " failed to add its menu items: " + ex.getMessage(), ex);
                }
            }
        }
	...
	private void showCoaEditor() {
		...
		CoaEditorPanelFX editor = new CoaEditorPanelFX(
			...
			chart -> {
				...
				try { CurrentCompany.persist(); } 
                catch (IOException | ActionCancelledException | NoFileCreatedException ex) { ex.printStackTrace(); }
			},
	...
	private void setState(AppState s) {
	    ...
	    this.miEditCompany.setDisable(creatingCompany); // Can edit if open, can create if no company
        ...
        // Plugin menus are added by plugins themselves. The core app enables/disables the top-level
        // "SCA Ledger" menu (or other plugin menus) if those plugins make that possible via ApplicationContext.
        // For now, top-level plugin menus added by plugins will be enabled/disabled along with "Run", "Reports", etc.
        // A more granular approach would involve plugins registering their top-level menus with the core app,
        // and the core app managing their disable state based on broader context like COMPANY_OPEN.
        // Or, plugins can check ApplicationContext.getCurrentCompany() themselves in their actions.
        // The SCALedgerPlugin's menu is added directly to the bar, so it won't be covered by this.run.setDisable().
        // It should ideally also be disabled if no company is open if its actions require one.
        // This can be handled by the plugin itself in its addMenuItems or if the main app manages plugin menus.
        // For now, SCALedgerPlugin's menu is always enabled once added. This is a V2 improvement area.
	}
	...
	@SuppressWarnings("unused") private void doCloseCompany() {
		try {
			CloseCompanyFileAction closeCompanyFileAction = new CloseCompanyFileAction(this.primaryStage);
			setState(AppState.NO_COMPANY);
		} catch (Exception e) {
			AlertBox.showError(null, e.getMessage());
		}
	...
	@SuppressWarnings("unused") private void doSaveCompany() {		
		try {
			SaveCompanyFileAction saveCompanyFileAction = new SaveCompanyFileAction(this.primaryStage);
	...
	@SuppressWarnings("unused") private void startCreateWizard() {
		...
		try {
			CreateOrEditCompanyActionFX createOrEditCompanyActionFX = new CreateOrEditCompanyActionFX(this.primaryStage);
			setState(AppState.COMPANY_OPEN);
		} catch (Exception e) {
			setState(saved);
			e.printStackTrace();
        ...
        if (this.loadedPlugins != null) {
            for (Plugin plugin : this.loadedPlugins) {
                try {
                    ...
                    LOGGER.log(Level.WARNING, "Error shutting down plugin: " + plugin.getName() + " - " + e.getMessage(), e);
                }
            }
        }
        super.stop(); 

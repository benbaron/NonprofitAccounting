...
public class GenerateIncomeStatementAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            ...
            // if (currentCompany.getChartOfAccounts() != null && currentCompany.getChartOfAccounts().getFunds() != null) {
            ...
            // }
            // Since neither Company nor ChartOfAccounts has getFunds(), availableFunds remains empty.
            ...
            JOptionPane.showMessageDialog(null, "Income Statement saved to: " + f.getAbsolutePath(), "Success", JOptionPane.INFORMATION_MESSAGE);

        } catch (Exception ex) {
            System.err.println("Error during Income Statement generation: " + ex.getMessage());
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null, "Failed to generate Income Statement: " + ex.getMessage(), "Generation Error", JOptionPane.ERROR_MESSAGE);
        } catch (IOException ex) { // Catch IOException from config saving specifically if not caught within block

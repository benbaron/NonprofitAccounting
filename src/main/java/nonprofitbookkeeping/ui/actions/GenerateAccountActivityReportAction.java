...
/**
 * JavaFX action handler for generating an Account Activity Detail report. This
 * action prompts the user for report criteria (date range, funds, accounts)
 * using a {@link ReportCriteriaDialog}, then uses the {@link ReportService} to
 * prepare the data and generate the report (typically as an XLSX file using
 * JXLS). It also handles saving the report configuration if a name is provided.
 */
public class GenerateAccountActivityReportAction implements EventHandler<ActionEvent>
{
	
	// private static final long serialVersionUID = 1L; // Not needed for
		// EventHandler
	...
	@Override public void handle(ActionEvent event)
	{
		...
		// companyDir can be null if companyFile is not set, saving config will be
		// skipped.
		
		List<Fund> availableFunds = new ArrayList<>(); // Placeholder, as Fund list isn't available
														// from Company/COA
		...
		try
		{
			...
			if (criteria.getSelectedAccountIds() == null ||
				criteria.getSelectedAccountIds().isEmpty())
			{
			...
			if (configNameToSave != null && !configNameToSave.trim().isEmpty())
			{
				...
				{
					AlertBox.showInfo(null,
						"Could not determine company directory. Configuration not saved."); // Changed
																							// from
																							// WARNING
																							// to
																							// INFO
		...
		{
			System.err.println(
				"IO Error related to report configuration loading/saving: " + ex.getMessage());
			ex.printStackTrace();
		...
		{
			System.err.println(
				"Error during Account Activity Detail report generation: " + ex.getMessage());
			ex.printStackTrace();

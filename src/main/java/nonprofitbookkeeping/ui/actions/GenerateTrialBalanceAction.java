...
/**
 * JavaFX action handler for generating a Trial Balance report. This class
 ...
 * <p>
 * The action prompts the user for report criteria (end date, optional start
 * date, funds) using a {@link ReportCriteriaDialog}, then utilizes the
 * {@link ReportService} to generate the report, typically as an XLSX file via
 * JXLS templates. It also supports saving the chosen report configuration.
 * </p>
 */
public class GenerateTrialBalanceAction extends AbstractAction implements EventHandler<ActionEvent>
{
	
	// private static final long serialVersionUID = 1L; // Not applicable for JavaFX
		// EventHandler
	...
	@Override public void handle(ActionEvent event)
	{
		...
		try
		{
			...
			Optional<ReportCriteria> criteriaOpt = ReportCriteriaDialog.showDialog(parentWindow,
				"Trial Balance Criteria", availableFunds, // This list of funds isn't directly used
															// by the dialog in this configuration
				null, // chartOfAccounts - not needed as account selector is false
				DateSelectionMode.DATE_RANGE_OPTIONAL_START, // End date is primary, start date is
																// optional

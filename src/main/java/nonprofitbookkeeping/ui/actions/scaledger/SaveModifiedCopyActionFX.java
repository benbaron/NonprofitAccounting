	...
	public void handle(ActionEvent e)
	{
		...
		if (this.plugin == null)
		{
			new Alert(Alert.AlertType.ERROR,
				"Plugin not initialized for SaveModifiedCopyActionFX.")
				.showAndWait();
			return;
		}
		...
		{
			new Alert(Alert.AlertType.ERROR,
				"No input file loaded or ledger selected.").showAndWait();
			return;
		}
		...
		chooser.getExtensionFilters()
			.add(new FileChooser.ExtensionFilter("Excel Macro-Enabled",
				"*.xlsm"));
		...
		{
			new Alert(Alert.AlertType.ERROR,
				"Please specify a file name, not a directory.").showAndWait();
			return;
		}
		...
		{
			new Alert(Alert.AlertType.ERROR,
				"Cannot write to the selected location.").showAndWait();
			return;
		}
		...
		try
		{
			...
			{
				new Alert(Alert.AlertType.ERROR,
					"PageViewerPanel not initialized in plugin.").showAndWait();
				return;
			}
			
			javax.swing.table.DefaultTableModel modelToSave =
				pvp.getTableModel();
			
			if (modelToSave == null)
			{
				new Alert(Alert.AlertType.ERROR,
					"Data model not available from PageViewerPanel.")
					.showAndWait();
				return;
			}

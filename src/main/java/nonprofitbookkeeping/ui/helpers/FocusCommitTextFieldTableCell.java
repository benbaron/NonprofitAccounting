...
public class FocusCommitTextFieldTableCell<S, T> extends TextFieldTableCell<S, T> {
    ...
    public void startEdit() {
        ...
        if (isEditing()) {
            TextField textField = (TextField) getGraphic(); // getGraphic() should return the TextField after super.startEdit()
            if (textField != null) {
                textField.focusedProperty().addListener((obs, oldFocusValue, newFocusValue) -> {
                    if (!newFocusValue) { // If focus is lost
                        ...
                        try {
                            ...
                            System.err.println("Conversion/Commit failed for value: " + text + ". Error: " + e.getMessage()); // Consider using a logger

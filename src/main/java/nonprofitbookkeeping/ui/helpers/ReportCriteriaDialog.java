...
public class ReportCriteriaDialog {
    ...
    public static Optional<ReportCriteria> showDialog(
        ...
        // Call the main overloaded method with null for initialConfig, chartOfAccounts, and false for showAccountSelector
        return showDialog(parentComponent, title, availableFunds, null, dateMode, showFundSelector, false, null);
    }

    // Overloaded method that takes initialConfig but not chartOfAccounts/showAccountSelector
    public static Optional<ReportCriteria> showDialog(
            Component parentComponent,
            String title,
            List<Fund> availableFunds,
        ...
        return showDialog(parentComponent, title, availableFunds, null, dateMode, showFundSelector, false, initialConfig);
    }
    ...
    private static class AccountItem {
        ...
        public String getId() { return account.getAccountNumber(); } // Using AccountNumber as ID
    ...
    public static Optional<ReportCriteria> showDialog(
        ...
        final DateSelectionMode actualDateMode = (initialConfig != null && initialConfig.getDateSelectionMode() != null) 
                                               ? initialConfig.getDateSelectionMode() 
                                               : dateMode;
        ...
        if (actualDateMode == DateSelectionMode.DATE_RANGE_MANDATORY_START || actualDateMode == DateSelectionMode.DATE_RANGE_OPTIONAL_START) {
            panel.add(new JLabel("Start Date:"), gbc);
            gbc.gridx++;
            JDatePanelImpl startDatePanel = new JDatePanelImpl(startDateModel, datePickerProps);
            startDatePicker = new JDatePickerImpl(startDatePanel, new JDatePickerImpl.JFormattedTextFieldFactory());
        ...
        endDatePicker = new JDatePickerImpl(endDatePanel, new JDatePickerImpl.JFormattedTextFieldFactory());
        ...
        if (showFundSelector) {
            ...
            if (!initiallySelectAllFunds && initialFundIds != null) {
                ...
                for (int i = 0; i < fundItems.length; i++) {
                    if (initialFundIds.contains(fundItems[i].getId())) indicesToSelect.add(i);
                }
                fundList.setSelectedIndices(indicesToSelect.stream().mapToInt(Integer::intValue).toArray());
            }
            ...
            if (selectAllFundsCheckbox != null) { // Check if checkbox was created
                selectAllFundsCheckbox.addItemListener(e -> {
                    if (finalFundList != null) {
                        ...
                        if (e.getStateChange() == ItemEvent.SELECTED) finalFundList.clearSelection();
                    }
                });
            }
        }
        ...
        if (showAccountSelector) {
            gbc.gridwidth = 2; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.gridx = 0;
            panel.add(new JSeparator(SwingConstants.HORIZONTAL), gbc);
            gbc.gridy++;
            ...
            List<Account> accounts = (chartOfAccounts != null && chartOfAccounts.getAccounts() != null) 
            ...
            if (initialConfig != null && initialConfig.getAccountIdsForDetailReport() != null && !initialConfig.getAccountIdsForDetailReport().isEmpty()) {
                ...
                accountListJList.setSelectedIndices(indicesToSelect.stream().mapToInt(Integer::intValue).toArray());
            }

            JScrollPane accountListScrollPane = new JScrollPane(accountListJList);
            fundListScrollPane.setPreferredSize(new Dimension(250, 100)); // Set preferred size for scroll pane
            gbc.gridwidth = 2; 
            gbc.fill = GridBagConstraints.HORIZONTAL;
            panel.add(fundListScrollPane, gbc);
            gbc.gridy++;
        ...
        Object[] options = {"Run Report", "Save Configuration...", "Cancel"};
        ...
        if (result == JOptionPane.YES_OPTION || result == JOptionPane.NO_OPTION) { // "Run Report" or "Save Configuration..."
            ...
            if (showFundSelector && selectAllFundsCheckbox != null && !selectAllFundsCheckbox.isSelected() && fundList != null) {
                selectedFundIds = fundList.getSelectedValuesList().stream()
                                      .map(FundItem::getId)
                                      .collect(Collectors.toList());
            } 
            // If "Select All Funds" is checked, selectedFundIds remains empty, signifying no specific fund filter.

            // Validation for dates (as before)
            if (actualDateMode == DateSelectionMode.SINGLE_DATE && endDate == null) {
                 JOptionPane.showMessageDialog(panel, "Date is required.", "Input Error", JOptionPane.ERROR_MESSAGE);
                 return showDialog(parentComponent, title, availableFunds, chartOfAccounts, actualDateMode, showFundSelector, showAccountSelector, initialConfig); 
            }
            if (actualDateMode == DateSelectionMode.DATE_RANGE_MANDATORY_START && (startDate == null || endDate == null)) {
                 JOptionPane.showMessageDialog(panel, "Both Start Date and End Date are required.", "Input Error", JOptionPane.ERROR_MESSAGE);
                 return showDialog(parentComponent, title, availableFunds, chartOfAccounts, actualDateMode, showFundSelector, showAccountSelector, initialConfig);
            }
            if (actualDateMode == DateSelectionMode.DATE_RANGE_OPTIONAL_START && endDate == null) {
                 JOptionPane.showMessageDialog(panel, "End Date is required.", "Input Error", JOptionPane.ERROR_MESSAGE);
                 return showDialog(parentComponent, title, availableFunds, chartOfAccounts, actualDateMode, showFundSelector, showAccountSelector, initialConfig);
            }
            if (startDate != null && endDate != null && endDate.isBefore(startDate)) {
                JOptionPane.showMessageDialog(panel, "End Date cannot be before Start Date.", "Input Error", JOptionPane.ERROR_MESSAGE);
                return showDialog(parentComponent, title, availableFunds, chartOfAccounts, actualDateMode, showFundSelector, showAccountSelector, initialConfig); 
            }

            List<String> selectedAccountIds = new ArrayList<>();
            if (showAccountSelector && accountListJList != null && accountListJList.getSelectedValuesList() != null) {
            ...
            if (result == JOptionPane.NO_OPTION) { // "Save Configuration..."
                String initialName = (initialConfig != null && initialConfig.getUserGivenName() != null) ? initialConfig.getUserGivenName() : "";
                String configName = JOptionPane.showInputDialog(panel, "Enter a name for this report configuration:", initialName);
                if (configName != null && !configName.trim().isEmpty()) {
                    ...
                    // For simplicity, if name input is cancelled, we still run the report without saving name.

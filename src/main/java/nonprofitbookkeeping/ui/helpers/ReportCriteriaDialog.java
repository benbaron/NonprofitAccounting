...
import nonprofitbookkeeping.reports.jasper.runtime.ReportCriteria;
...
/**
 * Utility class for displaying a JavaFX dialog to gather criteria for
 * generating reports. The dialog can be configured to show selectors for date
 * ranges (with various modes), funds, and accounts, based on the needs of the
 * specific report. It returns an {@link Optional} of {@link ReportCriteria}
 * allowing the caller to handle cases where the user cancels the dialog. The
 * dialog also supports pre-populating fields from an existing
 * {@link ReportConfiguration} and prompting the user to save the chosen
 * criteria as a new configuration.
 */
public class ReportCriteriaDialog
{
	
	/**
	 * Inner class to wrap a {@link Fund} object for display in UI controls like
	 * ListView. Overrides {@code toString()} to display the fund's name. Implements
	 * {@code equals()} and {@code hashCode()} based on the fund's ID for proper
	 * functioning in collections.
	 */

...
public class NonprofitBookkeepingSclxExportService
{
    ...
    public SclxDocument exportDocument() throws java.sql.SQLException
    {
        ...
        List<SclxDocument.Person> people = personRepository.list().stream()
            .map(person -> new SclxDocument.Person("person-" + person.getId(), person.getName(), "OTHER", person.getEmail(), person.getPhone(), Map.of()))
            .toList();

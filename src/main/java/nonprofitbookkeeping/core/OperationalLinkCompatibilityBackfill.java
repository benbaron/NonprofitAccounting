...
final class OperationalLinkCompatibilityBackfill
{
    void run(Connection c) throws SQLException
    {
        try (Statement st = c.createStatement())
        {
            linkMatchedDonations(st);
            insertDonationJournalLinks(st);
            linkMatchedGrants(st);
            queueUnmatchedDonations(st);
            queueUnmatchedGrants(st);
        }
    }

    private void linkMatchedDonations(Statement st) throws SQLException
    {
        st.execute("""
    ...
    private void insertDonationJournalLinks(Statement st) throws SQLException
    {
        st.execute("""
    ...
    private void linkMatchedGrants(Statement st) throws SQLException
    {
        st.execute("""
    ...
    private void queueUnmatchedDonations(Statement st) throws SQLException
    {
        st.execute("""
    ...
    private void queueUnmatchedGrants(Statement st) throws SQLException
    {
        st.execute("""
                INSERT INTO operational_link_backfill_queue(module_name, domain_id, issue_code, issue_detail)
                SELECT 'GRANT',

...
public class JacksonDataStorer implements DataStorer
{
                ...
                if (type == nonprofitbookkeeping.model.Company.class)
                {
                                ...
                                ZipInputStream zis = new ZipInputStream(fis))
                        {
                                ...
                                while ((zipEntry = zis.getNextEntry()) != null)
                                {
                                        ...
                                        {
                                                company = this.mapper.readValue(zis, nonprofitbookkeeping.model.Company.class);
                                        }
                                        else if (COA_ENTRY_NAME.equals(zipEntry.getName()))
                                        {
                                                chart = this.mapper.readValue(zis, nonprofitbookkeeping.model.ChartOfAccounts.class);
                                        }
                                        zis.closeEntry();
                                }
                                                                               ...
                                                                               NoFileCreatedException
        {
                ...
                if (obj instanceof nonprofitbookkeeping.model.Company)
                {
                        ...
                        try (ZipInputStream zis = file.exists() ? new ZipInputStream(new FileInputStream(file)) : null;
                             ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(temp)))
                        {
                                ...
                                if (zis != null)
                                {
                                        ...
                                        {
                                                if (!JSON_ENTRY_NAME.equals(e.getName()) && !COA_ENTRY_NAME.equals(e.getName()))
                                                {

...
public class AssetsRegisterPanel implements AppPanel
{
    ...
    public AssetsRegisterPanel()
    {
        ...
        table.getColumns().add(col("Depreciation %", AssetRow::depreciationRateProperty));
        table.getColumns().add(col("Useful Life (yrs)", AssetRow::usefulLifeYearsProperty));
        table.getColumns().add(col("Book Value", AssetRow::bookValueProperty));
        table.getItems().addAll(
            new AssetRow("Laptop Fleet", "Equipment", "2025-03-01", "4500.00", "20", "5", "3600.00"),
            new AssetRow("Office Furniture", "Furniture", "2024-05-15", "2800.00", "10", "10", "2400.00"));

        root.setCenter(table);
        root.setBottom(new VBox(new Separator(), status));

        add.setOnAction(
            e -> table.getItems().add(new AssetRow("New Asset", "", "", "0.00", "0", "0", "0.00")));
    ...
    public void onSave()
    {
        status.setText("Saved " + table.getItems().size() + " asset row(s) with inline depreciation fields");
    }
    ...
    public static final class AssetRow
    {
        ...
        public AssetRow(String asset, String category, String acquired, String cost, String depreciationRate,

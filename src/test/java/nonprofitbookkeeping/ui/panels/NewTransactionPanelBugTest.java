...
import nonprofitbookkeeping.java.AccountingTransaction; // Assuming this is the correct package
...
public class NewTransactionPanelBugTest {
    ...
    private void start(Stage stage) {
        ...
        newTransactionPanel = new NewTransactionPanelFX(mockOnSave);
        
        Scene scene = new Scene(newTransactionPanel, 800, 600);
        stage.setScene(scene);
        stage.show();
        stage.toFront();

        // Lookup UI elements after the stage is shown
        linesTable = robot.lookup(".table-view").queryTableView(); // Generic lookup for TableView
        memoTextArea = robot.lookup(".text-area").queryAs(TextArea.class); // Generic lookup for TextArea
    }
    ...
    void reproduceDataErasureBug(FxRobot robot) {
        ...
        Platform.runLater(() -> {
            linesTable.getItems().add(new Line()); // Add directly to TableView's items
            linesTable.refresh(); // Ensure UI updates
        });
        WaitForAsyncUtils.waitForFxEvents(); // Wait for UI updates

        // Verify line was added
        Assertions.assertThat(linesTable.getItems()).hasSize(1);
        Line addedLine = linesTable.getItems().get(0);
        ...
        robot.doubleClickOn(cellQuery.query()); // Double click to start editing
        ...
        Assertions.assertThat(memoTextArea).isNotNull(); // Ensure memoTextArea was found
        robot.clickOn(memoTextArea);
        WaitForAsyncUtils.waitForFxEvents();

        // Step 7: Retrieve the value from the cell's underlying model item
        Line firstLine = linesTable.getItems().get(0);

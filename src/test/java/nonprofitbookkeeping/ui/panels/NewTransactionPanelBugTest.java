...
@ExtendWith(ApplicationExtension.class) public class NewTransactionPanelBugTest
{
	...
	@Start private void start(Stage stage)
	{
		...
		this.linesTable = this.robot.lookup(".table-view").queryTableView(); // Generic lookup for
																				// TableView
		this.memoTextArea = this.robot.lookup(".text-area").queryAs(TextArea.class); // Generic
																						// lookup
																						// for
																						// TextArea
	}
	
	// Helper method to access robot in @Start, TestFX injects it into test methods
	// but not @Start
	// This is a common pattern if you need robot during setup.
	// However, for this test, we can pass robot to methods that need it or do
	// lookups in the test method itself.
	// For simplicity, I'll perform lookups in start and make sure they are valid.
	// If robot is needed in start, it would require ApplicationTest.launch() or
	// similar setup.
	// Let's assume TestFX setup handles robot availability for lookups in @Start
	// too,
	// if not, these lookups would be moved to the beginning of the test method.
	// TestFX's ApplicationExtension should make robot available for injection in
		...
		void reproduceDataErasureBug(FxRobot robot1)
	{
		...
		org.assertj.core.api.Assertions.assertThat(addedLine.amount.get())
		...
		org.assertj.core.api.Assertions.assertThat(rowQuery.queryAll()).isNotEmpty(); // Make sure
																						// row is
																						// there
		
		org.testfx.service.query.NodeQuery cellQuery = rowQuery.lookup(".table-cell").nth(2);
		
		org.assertj.core.api.Assertions.assertThat(cellQuery.queryAll()).isNotEmpty(); // Make sure
																						// cell is
																						// there
		
// robot1.doubleClickOn(cellQuery.query()); // Corrected to double click on the
// actual cell
		...
		// Step 6: Click on another UI element (Memo TextArea) to make the cell lose
		// focus
		...
		// Step 8: Assert that the value has reverted to BigDecimal.ZERO (the default
		// for new Line's amount)
		...
		org.assertj.core.api.Assertions.assertThat(amountAfterLosingFocus)
			...
			.isEqualTo(BigDecimal.ZERO);
		// If it was not BigDecimal.ZERO but some other default, this would need to
		// change.

...
import static org.testfx.matcher.control.TextInputControlMatchers.hasText as hasTextInField;
...
public class CreateOrEditCompanyPanelFXTest extends JavaFXTestBase {
    ...
    public void start(Stage stage) throws Exception {
        newCompany = new Company(); // For creating a new company
        mockCallback = mock(CompanyCreatedCallback.class);
        panel = new CreateOrEditCompanyPanelFX(newCompany, mockCallback);
    ...
    public void testInitialState_Step1Visible_NavigationButtonsCorrect() {
        ...
        verifyThat("Company Information", (TitledPane pane) -> pane.isVisible() && pane.isExpanded());

        // Check fields for step 1 (presence)
        lookup((Node node) -> node instanceof TextField && "Company Name:".equals(findLabelForNode(node))).queryAs(TextField.class);
    ...
    @Test
    public void testNavigation_NextAndBackButtons() {
        // Step 1 to Step 2
        clickOn("Next");
        ...
        verifyThat("Back", isEnabled());
        verifyThat("Next", isDisabled()); // Next should be disabled on the last step (replaced by Save)
    ...
    private String findLabelForNode(Node node) {
        ...
        if (node.getParent() instanceof GridPane) {
            ...
            if (colIndex != null && rowIndex != null && colIndex > 0) {
                for (Node child : grid.getChildren()) {
                    if (GridPane.getRowIndex(child) != null && GridPane.getRowIndex(child).equals(rowIndex) &&
                        GridPane.getColumnIndex(child) != null && GridPane.getColumnIndex(child).equals(colIndex - 1) &&
    ...
    @Test
    public void testFillAndSaveForm_VerifyCallbackData() {
        // Step 1: Company Information
        ...
        // Fill Step 1
        ((TextField)lookup((Node node) -> node instanceof TextField && "Company Name:".equals(findLabelForNode(node))).query()).setText(COMPANY_NAME);
        selectInComboBox(findComboBoxByLabel("Legal Structure:"), LEGAL_STRUCTURE);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Tax ID:".equals(findLabelForNode(node))).query()).setText(TAX_ID);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Address:".equals(findLabelForNode(node))).query()).setText(ADDRESS);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Phone:".equals(findLabelForNode(node))).query()).setText(PHONE);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Email:".equals(findLabelForNode(node))).query()).setText(EMAIL);
        clickOn("Next");
        WaitForAsyncUtils.waitForFxEvents();

        // Fill Step 2
        ((TextField)lookup((Node node) -> node instanceof TextField && "Fiscal Year Start:".equals(findLabelForNode(node))).query()).setText(FISCAL_START);
        selectInComboBox(findComboBoxByLabel("Base Currency:"), CURRENCY);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Starting Balance Date:".equals(findLabelForNode(node))).query()).setText(START_BALANCE_DATE);
        selectInComboBox(findComboBoxByLabel("Chart of Accounts:"), CHART_OF_ACCOUNTS);
        clickOn("Next");
        WaitForAsyncUtils.waitForFxEvents();

        // Fill Step 3
        ((TextField)lookup((Node node) -> node instanceof TextField && "Admin Username:".equals(findLabelForNode(node))).query()).setText(ADMIN_USER);
        ((PasswordField)lookup((Node node) -> node instanceof PasswordField && "Admin Password:".equals(findLabelForNode(node))).query()).setText(ADMIN_PASS);
        ((TextField)lookup((Node node) -> node instanceof TextField && "Default Bank Account:".equals(findLabelForNode(node))).query()).setText(DEFAULT_BANK);

        // Checkboxes - assuming fx:id or direct lookup possible if IDs were set.
        // If not, lookup by text.
        clickOn((CheckBox) lookup(".check-box").match(cb -> ((CheckBox)cb).getText().equals("Enable Fund Accounting")).query()); // Example: click to change state
        clickOn((CheckBox) lookup(".check-box").match(cb -> ((CheckBox)cb).getText().equals("Enable Inventory Tracking")).query());
        ...
        // Verify callback
        ArgumentCaptor<CompanyProfileModel> captor = ArgumentCaptor.forClass(CompanyProfileModel.class);
        verify(mockCallback, times(1)).onCreatedProfileModel(captor.capture());
        ...
        // Assuming multiCurBox was not clicked and defaults to false or its initial state
        // boolean expectedMultiCurrency = ((CheckBox) lookup(".check-box").match(cb -> ((CheckBox)cb).getText().equals("Enable Multi-Currency")).query()).isSelected();
    ...
    private ComboBox<String> findComboBoxByLabel(String labelText) {
        return lookup((Node node) -> node instanceof ComboBox && labelText.equals(findLabelForNode(node)))
                .queryComboBox();
    }

    private void selectInComboBox(ComboBox<String> comboBox, String valueToSelect) {
        Platform.runLater(() -> comboBox.setValue(valueToSelect));
        WaitForAsyncUtils.waitForFxEvents();
    }

    // TODO: Add tests for input validation if any specific validation rules are implemented in the panel.

...
import static org.testfx.matcher.control.TreeViewMatchers.hasNumRows; // For TreeTableView, this is an approximation
import static org.testfx.matcher.control.TreeViewMatchers.hasChildren;
...
public class CoaEditorPanelFXTest extends JavaFXTestBase {
    ...
    public void start(Stage stage) throws Exception {
        ...
        testCoa.addRootAccount(rootAsset);

        Account rootEquity = new Account("3000", "Equity", AccountType.EQUITY, BigDecimal.ZERO);
        testCoa.addRootAccount(rootEquity);

        mockOnSave = mock(Consumer.class);
        mockOnClose = mock(Runnable.class);

        panel = new CoaEditorPanelFX(testCoa, mockOnSave, mockOnClose);
        Scene scene = new Scene(panel, 800, 600);
        stage.setScene(scene);
        stage.show();

        tree = lookup(".tree-table-view").query();
    }
    ...
    @Test
    public void testInitialDisplay_ShowsAccounts() {
        assertNotNull(tree);
        ...
        assertEquals(3, tree.getRoot().getChildren().stream().mapToInt(ti -> 1 + ti.getChildren().size()).sum());

        TreeItem<Account> assetsItem = findTreeItem(tree.getRoot(), "Assets");
    ...
    @Test
    public void testAddRootAccount_Successful() {
        clickOn("Add Root");
        WaitForAsyncUtils.waitForFxEvents();
        ...
        TreeItem<Account> liabilitiesItem = findTreeItem(tree.getRoot(), "Liabilities");
        assertNotNull(liabilitiesItem);
        assertEquals("4000", liabilitiesItem.getValue().getAccountNumber());
        assertEquals(BigDecimal.valueOf(500.00).stripTrailingZeros(), liabilitiesItem.getValue().getOpeningBalance().stripTrailingZeros());
    }

    @Test
    public void testAddSubAccount_Successful() {
        TreeItem<Account> assetsItem = selectTreeItem(tree, "Assets"); // Select "Assets"
    ...
    public void testEditAccount_Successful() {
        TreeItem<Account> bankItem = selectTreeItem(tree, "Bank");
        ...
        assertEquals(BigDecimal.valueOf(1200.00).stripTrailingZeros(), bankItem.getValue().getOpeningBalance().stripTrailingZeros());
        tree.refresh(); // Ensure UI reflects change
        assertEquals("Main Bank Account", bankItem.getValue().getName()); // Double check after refresh
    }

    @Test
    public void testDeleteAccount_Successful() {
        selectTreeItem(tree, "Bank"); // Select "Bank"
        clickOn("Delete");
    ...
    @Test
    public void testSaveButton_InvokesCallback() {
        clickOn("Save");
        WaitForAsyncUtils.waitForFxEvents();
        ...
        verify(mockOnClose, times(1)).run(); // onClose is also called

        ChartOfAccounts savedCoa = captor.getValue();
        assertNotNull(savedCoa);
        // Could add more assertions about the state of savedCoa if specific changes were made
    }

    @Test
    public void testCancelButton_InvokesCallback() {
        clickOn("Cancel");
        WaitForAsyncUtils.waitForFxEvents();

        verify(mockOnSave, never()).accept(any());
    ...
    private void fillAccountDialog(String number, String name, AccountType type, String balance) {
        ...
        if (number != null) {
            TextField numField = from(dialogPane).lookup(".text-field").nth(0).query(); // Assumes order
            numField.setText(number);
        }
        if (name != null) {
            TextField nameField = from(dialogPane).lookup(".text-field").nth(number == null ? 0 : 1).query(); // Adjust index if number field was skipped
            nameField.setText(name);
        }
        if (type != null) {
            @SuppressWarnings("unchecked")
            ComboBox<AccountType> typeCombo = from(dialogPane).lookup(".combo-box").queryComboBox();
            Platform.runLater(() -> typeCombo.setValue(type));
            WaitForAsyncUtils.waitForFxEvents();
        }
        if (balance != null) {
             List<Node> textFields = from(dialogPane).lookup(".text-field").queryAll().stream().collect(Collectors.toList());

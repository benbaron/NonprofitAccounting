...
public class BudgetEditorDialogFXTest extends JavaFXTestBase
{
	...
	private void showDialog(Budget budgetToEdit)
	{
		Platform.runLater(() -> {
			this.dialog =
				new BudgetEditorDialogFX(this.ownerStage, this.mockCoa, this.availableFunds,
					this.mockBudgetService, this.mockCompanyDirectory, budgetToEdit);
	...
	{
		if (dialogPane == null || dialogPane.getContent() == null)
			return;
		...
		if (topContent instanceof GridPane)
		{
			...
			if (propertiesGrid.getChildren().size() > 1 &&
				propertiesGrid.getChildren().get(1) instanceof TextField)
				((TextField) propertiesGrid.getChildren().get(1)).setId("budgetNameField");
			if (propertiesGrid.getChildren().size() > 3 &&
				propertiesGrid.getChildren().get(3) instanceof Spinner)
				((Spinner<?>) propertiesGrid.getChildren().get(3)).setId("fiscalYearSpinner");
			if (propertiesGrid.getChildren().size() > 5 &&
				propertiesGrid.getChildren().get(5) instanceof TextField)
				((TextField) propertiesGrid.getChildren().get(5)).setId("descriptionField");
			if (propertiesGrid.getChildren().size() > 7 &&
				propertiesGrid.getChildren().get(7) instanceof ComboBox)
				((ComboBox<?>) propertiesGrid.getChildren().get(7)).setId("applicableFundComboBox");
			if (propertiesGrid.getChildren().size() > 9 &&
				propertiesGrid.getChildren().get(9) instanceof TextField)
				((TextField) propertiesGrid.getChildren().get(9)).setId("currencyField");
		}
		...
		{
			Optional<Node> tableOpt = ((VBox) centerContent).getChildren().stream()
				.filter(n -> n instanceof TableView).findFirst();
			tableOpt.ifPresent(node -> node.setId("budgetLinesTable"));
		}
		
	}
	
	@AfterEach public void closeDialogAfterTest()
	{
	...
	@Test public void testCreateMode_InitialStateAndFields()
	{
		showDialog(null);
		...
		// lookup("#budgetNameField")..queryAs(TextField.class);
		...
		assertNotNull(lookup("#budgetLinesTable").queryTableView(),
			"Budget lines table should exist.");
		...
		assertNotNull(this.dialog.getDialogPane()
			.lookupButton(new ButtonType("Save Budget", ButtonBar.ButtonData.OK_DONE)));
		assertNotNull(this.dialog.getDialogPane().lookupButton(ButtonType.CANCEL));
	}
	
	@Test public void testEditMode_PopulatesFieldsFromExistingBudget()
	{
		...
		// assertEquals("Annual Operations",
		// lookup("#budgetNameField")..queryAs(TextField.class).getText());
		assertEquals(2023, ((Spinner<Integer>) lookup("#fiscalYearSpinner").queryAs(Spinner.class))
			.getValue().intValue());
		assertEquals("Operational budget for 2023",
			lookup("#descriptionField").queryAs(TextField.class).getText());
		// assertEquals("USD",
		// lookup("#currencyField")..queryAs(TextField.class).getText());
	...
	@Test public void testFieldInteractions_ValuesChange()
	{
		showDialog(null);
	...
	@Test public void testSaveButton_ClosesDialog()
	{
		showDialog(null);
		
		Button saveButton = (Button) this.dialog.getDialogPane()
			.lookupButton(new ButtonType("Save Budget", ButtonBar.ButtonData.OK_DONE));
		clickOn(saveButton);
		WaitForAsyncUtils.waitForFxEvents();
	...
	@Test public void testCancelButton_ClosesDialog()
	{
		showDialog(null);

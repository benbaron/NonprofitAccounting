...
import nonprofitbookkeeping.model.Company; // Assuming Company is a concrete
											// class that can be instantiated
import nonprofitbookkeeping.model.CompanyProfileModel; // Company has a
														// CompanyProfileModel
import nonprofitbookkeeping.model.CurrentCompany;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.io.TempDir; // For creating temporary
											// files/directories
...
public class JacksonDataStorerZipTest
{
	...
	/** The temp dir. */
	@TempDir Path tempDir; // JUnit 5 temporary directory
	
	/**
	 * Test save and load zip format.
	 *
	...
	void testSaveAndLoadZipFormat() throws IOException
	{
		...
		originalCompany.setCompanyProfileModel(profile); // Corrected method
															// name
		...
		entries.add(new AccountingEntry(new BigDecimal("10.00"), "100",
			AccountSide.DEBIT));
		entries.add(new AccountingEntry(new BigDecimal("10.00"), "200",
			AccountSide.CREDIT));
		...
		// Add more simple, serializable data to the company if needed for a
		// more robust test
		...
		try
		{
			...
			// 3. Verify that the created file is a valid zip file and contains
			// "company_data.json"
			assertTrue(testFile.exists(),
				"Test file should exist after saving.");
			...
			assertTrue(foundJsonEntry,
				"Zip file should contain 'company_data.json' entry.");
			assertTrue(foundCoaEntry,
				"Zip file should contain 'chart_of_accounts.json' entry.");
			assertTrue(foundProfileEntry,
				"Zip file should contain 'company_profile.json' entry.");
			assertTrue(foundLedgerEntry,
				"Zip file should contain 'ledger.json' entry.");
			
			// 4. Use JacksonDataStorer.loadData() to load the Company object
			// back
			...
			assertNotNull(loadedCompany.getCompanyProfileModel(),
				"Loaded company profile should not be null."); // Corrected
																// method name
			assertNotNull(loadedCompany.getChartOfAccounts(),
				"Chart should load from separate entry.");
			assertNotNull(loadedCompany.getLedger(),
				"Ledger should be reconstructed from dedicated entry.");
			assertEquals(1,
				loadedCompany.getLedger().getJournal().getJournalTransactions()
					.size(),
				"Ledger transactions should round-trip through modular archive entries.");
			assertEquals(
				originalCompany.getCompanyProfileModel().getCompanyName(),
				loadedCompany.getCompanyProfileModel().getCompanyName(),
				"Company names should match."); // Corrected method name
			assertEquals(
				originalCompany.getCompanyProfileModel().getFiscalYearStart(),
				loadedCompany.getCompanyProfileModel().getFiscalYearStart(),
				"Fiscal year starts should match."); // Corrected method name
			assertEquals(
				originalCompany.getCompanyProfileModel().getBaseCurrency(),
				loadedCompany.getCompanyProfileModel().getBaseCurrency(),
				"Currencies should match."); // Corrected method name
	...
	/**
	 * Test load chart of accounts from zip.
	 *
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	@Test
	void testLoadChartOfAccountsFromZip() throws IOException
	{
		...
		try
		{
			...
			{
				loadedChart =
					dataStorer.loadData(ChartOfAccounts.class, testFile);
			}
			catch (Exception e)
			{
			...
			assertEquals("101",
				loadedChart.getRootAccounts().get(0).getAccountNumber());
		}
		finally
		{
			CurrentCompany.forceCompanyLoad(null);

...
class ReconciliationServiceTest
{
	...
	void setUp()
	{
		...
		Account income =
			new Account(OFFSET_ACCOUNT, "Contributions", AccountSide.CREDIT);
		...
		this.januaryDeposit = createTransaction(new BigDecimal("125.00"), 1L,
			"2024-01-05", "Initial deposit");
		this.februaryPayment = createTransaction(new BigDecimal("80.00"), 2L,
			"2024-02-02", "Sponsor payment");
	...
	{
		List<AccountingTransaction> unreconciled =
			ReconciliationService.getUnreconciled(BANK_ACCOUNT);
	...
	{
		this.service.reconcile(BANK_ACCOUNT, "2024-02-28",
			new BigDecimal("205.00"), List.of(1L));
		
		List<AccountingTransaction> remaining =
			ReconciliationService.getUnreconciled(BANK_ACCOUNT);
	...
	{
		AccountingTransaction manual = createTransaction(
			new BigDecimal("30.00"), 3L, "2024-03-10", "Manual entry");
		this.ledger.getJournal().deleteTransaction(3L); // manual transaction
														// should not live in
														// ledger yet
	...
	{
		List<String[]> rows = ReconciliationService
			.getUnreconciledEntries(BANK_ACCOUNT, "2024-02-01", "2024-02-28");
	...
	private AccountingTransaction createTransaction(BigDecimal amount, long id,
		String date, String memo)
	{

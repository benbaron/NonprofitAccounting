...
@ExtendWith(MockitoExtension.class) class ReportServiceTest
{
		...
		void setUp()
	{
		...
		// Account Mocks
		setupAccountMock(mockIncomeAccount1, "Donations", "INC100", AccountType.INCOME,
			AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockIncomeAccount2, "Grants", "INC200", AccountType.INCOME,
			AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockExpenseAccount1, "Rent", "EXP100", AccountType.EXPENSE,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockExpenseAccount2, "Utilities", "EXP200", AccountType.EXPENSE,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAssetAccount, "AssetAccount1", "ASSET100", AccountType.ASSET,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAssetAccount2, "AssetAccount2", "ASSET102", AccountType.ASSET,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAssetAccount3_MultiFund, "AssetAccount3-MultiFund", "ASSET103",
			AccountType.ASSET, AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAssetAccount4_NoFund, "AssetAccount4-NoFund", "ASSET104",
			AccountType.ASSET, AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockLiabilityAccount1, "LiabilityAccount1", "LIAB100",
			AccountType.LIABILITY, AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockEquityAccount1, "EquityAccount1", "EQUITY100", AccountType.EQUITY,
			AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockCashAccount1, "Main Bank Account", "CASH100", AccountType.BANK,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockCashAccount2, "Petty Cash", "CASH110", AccountType.CASH,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockDepreciationExpenseAccount, "Depreciation Expense", "EXPDEP100",
			AccountType.EXPENSE, AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAccumulatedDepreciationAccount, "Accumulated Depreciation",
			"ASSETCA100", AccountType.ASSET, AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockAccountsReceivableAccount, "Accounts Receivable", "AR100",
			AccountType.ASSET, AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockInventoryAccount, "Inventory", "INV100", AccountType.ASSET,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockAccountsPayableAccount, "Accounts Payable", "AP100",
			AccountType.LIABILITY, AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockFixedAssetAccount, "Equipment", "FA100", AccountType.FIXED_ASSET,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockFixedAssetAccount2_FundB, "Building", "FA200", AccountType.FIXED_ASSET,
			AccountSide.DEBIT, BigDecimal.ZERO);
		setupAccountMock(mockLongTermLoanAccount, "LT Loan Payable", "LTLOAN100",
			AccountType.LONG_TERM_LIABILITY, AccountSide.CREDIT, BigDecimal.ZERO);
		setupAccountMock(mockLoanAccount2_FundB, "Mortgage Payable", "LTLOAN200",
			AccountType.LONG_TERM_LIABILITY, AccountSide.CREDIT, BigDecimal.ZERO);
		...
		List<Account> allAccounts = new ArrayList<>(List.of(
			...
			mockAssetAccount, mockAssetAccount2, mockAssetAccount3_MultiFund,
			mockAssetAccount4_NoFund,
			...
			mockFixedAssetAccount, mockFixedAssetAccount2_FundB, mockLongTermLoanAccount,
			mockLoanAccount2_FundB));
		lenient().when(mockChartOfAccounts.getAccounts()).thenReturn(allAccounts);
		lenient().when(mockLedger.getTransactions()).thenReturn(null);
	}
	...
	private AccountingTransaction createMockTransaction(long timestamp,
														Set<AccountingEntry> entries)
	{
	...
	private void findItemAndAssert_TrialBalance(List<Map<String, Object>> items, String accNum,
												String accName, BigDecimal debit, BigDecimal credit)
	{
		/* ... */ }
		
	private void findBvAItemAndAssert(	List<Map<String, Object>> items, String accountId,
										BigDecimal budgeted, BigDecimal actual, BigDecimal variance,
										BigDecimal variancePercent)
	{
		/* ... */ }
		
	private void assertBalanceSheetItems(	Map<String, Object> context, String accountName,
											BigDecimal expectedAmount, String itemListName)
	{
		/* ... */ }
		
	private boolean accountPresentInBSItems(Map<String, Object> context, String accountName,
											String itemListName)
	{
		return false;
	}
	...
	private Map<String, Object> getAccountData(	List<Map<String, Object>> accountsDetail,
		...
		void testAA_NoAccountsSelected()
	{
		...
		Exception exception = assertThrows(IllegalArgumentException.class, () -> {
			ReportService.prepareAccountActivityContext(aaReportContext, mockLedger,
				mockChartOfAccounts);
		});
		assertTrue(
		...
		void testAA_AccountNotFound()
	{
		...
		Map<String, Object> context = ReportService.prepareAccountActivityContext(aaReportContext,
			mockLedger, mockChartOfAccounts);
		...
		void testAA_SingleAccount_NoTransactions()
	{
			...
			Mockito.mockStatic(ReportService.class, Mockito.CALLS_REAL_METHODS))
		{
			mockedStaticReportService.when(() -> ReportService.getAccountBalanceAsOfDate(
				eq(mockAssetAccount), eq(startDate.minusDays(1)), eq(mockLedger),
				eq(mockChartOfAccounts), any(), anyBoolean()))
				.thenReturn(openingBalance);
			when(mockLedger.getTransactions()).thenReturn(null);
			
			Map<String, Object> context = ReportService
				.prepareAccountActivityContext(aaReportContext, mockLedger, mockChartOfAccounts);
			List<Map<String, Object>> accountsDetail =
				(List<Map<String, Object>>) context.get("accountsDetail");
		...
		void testAA_SingleAccount_WithTransactions_DebitNormal()
	{
		...
		AccountingTransaction tx1 =
			createMockTransaction(startDateMillis, "TX1", "Tx Memo 1", Set.of(entry1));
		AccountingEntry entry2 =
			new AccountingEntry(new BigDecimal("20.00"), "ASSET100", AccountSide.CREDIT);
		AccountingTransaction tx2 =
			createMockTransaction(startDateMillis + 1000, "TX2", "Tx Memo 2", Set.of(entry2)); // Ensure
																								// different
																								// timestamp
		...
		void testAA_SingleAccount_WithTransactions_CreditNormal()
	{
		...
		AccountingTransaction tx1 =
			createMockTransaction(startDateMillis, "TX1", "Credit Entry", Set.of(entry1));
		AccountingEntry entry2 =
			new AccountingEntry(new BigDecimal("30.00"), "LIAB100", AccountSide.DEBIT);
		AccountingTransaction tx2 =
			createMockTransaction(startDateMillis + 1000, "TX2", "Debit Entry", Set.of(entry2));
		...
		void testAA_DateFilteringOfTransactions()
	{
		...
		AccountingTransaction txBefore = createMockTransaction(
		...
		AccountingTransaction txDuring = createMockTransaction(
		...
		AccountingTransaction txAfter = createMockTransaction(
			...
			Mockito.mockStatic(ReportService.class, Mockito.CALLS_REAL_METHODS))
		{
			...
			mockedStaticReportService.when(() -> ReportService.getAccountBalanceAsOfDate(
				eq(mockAssetAccount), eq(aaReportContext.getStartDate().minusDays(1)),
				eq(mockLedger), eq(mockChartOfAccounts), any(), anyBoolean()))
	...
	{
		when(mockAssetAccount.getAssociatedFunds()).thenReturn(List.of(mockFundB)); // Does not
																					// match FundA
		aaReportContext.setAccountIdsForDetailReport(List.of("ASSET100"));
		...
		void testAA_WithFundFilterActive_AccountIncluded_OBFundAware()
	{
			...
			Mockito.mockStatic(ReportService.class, Mockito.CALLS_REAL_METHODS))
		{
			...
			mockedStaticReportService.when(() -> ReportService.getAccountBalanceAsOfDate(
				eq(mockAssetAccount), eq(startDate.minusDays(1)), eq(mockLedger),
				eq(mockChartOfAccounts), eq(List.of("Operations")), eq(true)))

...
- Review Swing stub methods like `performAction()` in report actions and implement or remove them if unused. *(complete – GenerateReportsAction now delegates Swing listeners to the JavaFX handler, eliminating the dormant stub methods.)*
- Move remaining company subsections into separate JSON files inside the `.npbk` archive for modular storage. *(complete – JacksonDataStorer emits `company_profile.json`, `ledger.json`, and `chart_of_accounts.json` alongside a compatibility payload and the importer/migrator reassembles these sections.)*
